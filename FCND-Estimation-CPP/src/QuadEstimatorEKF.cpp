...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, yawEst);
  attitude.IntegrateBodyRate(gyro, dtIMU);   // the multiplication is already done in IntegrateBodyRate()!
  ...
  rollEst = attitudeTau / (attitudeTau + dtIMU) * (attitude.Roll())+dtIMU / (attitudeTau + dtIMU) * accelRoll;
  pitchEst = attitudeTau / (attitudeTau + dtIMU) * (attitude.Pitch())+dtIMU / (attitudeTau + dtIMU) * accelPitch;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  //         Not needed - is this confusion on purpose?!
  //   
  // OUTPUT:
  ...
  // Implement equation (49)

  V3F accelI = attitude.Rotate_BtoI(accel);
  
  accelI.z -= static_cast<float>(CONST_GRAVITY);
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  //
  // This is equation (52) in https://www.overleaf.com/project/5c34caab7ecefc04087273b9
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // fill gPrime according to eq (51) 

  V3F u_t = accel;  // just for comparability w/ the paper

  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = dt * (RbgPrime(0) * u_t).sum();
  gPrime(4, 6) = dt * (RbgPrime(1) * u_t).sum();
  gPrime(5, 6) = dt * (RbgPrime(2) * u_t).sum();
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  hPrime(0, QUAD_EKF_NUM_STATES-1) = 1;  // eq 58
